...
export default function TailwindScrollPlayground() {
    ...
    const containerClasses = `flex overflow-x-auto w-full max-w-2xl h-64 rounded-xl border border-foreground/10 bg-black/40 ${scrollBehavior} ${snapType} ${snapStrictness} ${scrollPadding} custom-scrollbar`;
    const itemClasses = `flex-shrink-0 w-48 h-full flex flex-col items-center justify-center text-foreground font-bold text-2xl ${snapAlign} ${snapStop} ${scrollMargin} transition-all duration-300`;
    ...
    return (
        <div className="min-h-screen bg-background text-foreground selection:bg-cyan-500/30 font-sans">
            ...
            <div className="pt-24 pb-20 px-4 md:px-8 max-w-7xl mx-auto flex flex-col lg:flex-row gap-8 min-h-[calc(100vh-100px)]">
                ...
                >
                    <div className="flex-1 min-h-[400px] rounded-2xl border border-foreground/10 relative overflow-hidden bg-[#0a0a0a] p-8 flex flex-col items-center justify-center gap-8">
                        <div className="absolute inset-0 bg-[linear-gradient(to_right,#80808012_1px,transparent_1px),linear-gradient(to_bottom,#80808012_1px,transparent_1px)] bg-size-[24px_24px]" />

                        <div className="text-gray-600 dark:text-gray-400 text-sm max-w-md text-center">
                        ...
                        <div className="flex gap-2 z-10">
                            {[0, 1, 2, 3, 4].map((i) => (
                                <button
                                    ...
                                    className="px-3 py-1 bg-foreground/5 border border-foreground/10 rounded-md text-xs hover:bg-white/10 transition-colors"
                                >
                                    Item {i + 1}
                                </button>
                            ))}
                    ...
                    <div className="bg-[#0a0a0a] border border-foreground/10 rounded-xl overflow-hidden">
                        <div className="flex items-center justify-between px-4 py-3 border-b border-foreground/5 bg-foreground/5">
                            <span className="text-xs font-medium text-gray-600 dark:text-gray-400">Class Output</span>
                            <button
                                onClick={handleCopy}
...
function SelectControl({ value, onChange, options }: { value: string, onChange: (val: string) => void, options: string[] }) {
    return (
        <select
            ...
            className="w-full bg-black/20 border border-foreground/10 rounded-lg text-sm text-gray-700 dark:text-gray-300 p-2 outline-none focus:border-cyan-500/50 cursor-pointer transition-colors hover:border-white/20"
        >
            {options.map((opt) => (
