...
function Hit({ hit, onClose }: any) {
  return (
    <Link
      ...
      onClick={onClose}
      className="block rounded-md px-4 py-3 hover:bg-foreground/5 transition"
    >
  ...
  return (
    ...
    >
      <div
        ...
        className="relative mx-auto mt-24 max-w-2xl rounded-xl bg-[#050014] border border-foreground/10 shadow-2xl"
      >
        <InstantSearchNext searchClient={searchClient}>
          <Index indexName={process.env.NEXT_PUBLIC_ALGOLIA_INDEX_NAME!}>
            <div
              ...
              className="border-b border-foreground/10 px-4 py-3"
            >
              <SearchBox
                placeholder="Search Codophile..."
                classNames={{
                  input:
                    "w-full bg-transparent text-foreground outline-none placeholder-gray-500",
                }}
              />
            </div>
